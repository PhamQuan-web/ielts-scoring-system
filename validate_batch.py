...
for i, line in enumerate(open(FILE, encoding="utf-8"), 1):
    ...
    if p == 3 and (wc < 55 or wc > 125): print(f"ERR L{i}: Part 3 has {wc}w (needs 60-120)"); errs += 1
...
# Check unique transcripts (must be unique)
transcripts = [s["transcript_cleaned"] for s in samples]
t_dupes = [k for k,v in Counter(transcripts).items() if v > 1]
if t_dupes: print(f"ERR: {len(t_dupes)} duplicate Transcripts"); errs += len(t_dupes)
